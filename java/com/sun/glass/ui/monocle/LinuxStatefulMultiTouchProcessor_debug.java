...
class LinuxStatefulMultiTouchProcessor extends LinuxTouchProcessor {
    ...
    public void processEvents(LinuxInputDevice device) {
        ...
        boolean eventHasMeaningfulData = false;
        while (buffer.hasNextEvent()) {
            String DEBUG_TYPE;
            System.out.println("Event " + (DEBUG_TYPE = LinuxInput.typeToString(buffer.getEventType())));
            switch (buffer.getEventType()) {
                case LinuxInput.EV_ABS: {
                    int value = transform.getValue(buffer);
                    System.out.println("Axis " + LinuxInput.codeToString(DEBUG_TYPE, (short) transform.getAxis(buffer)));
                    switch (transform.getAxis(buffer)) {
                        case LinuxInput.ABS_MT_SLOT:
                            eventHasMeaningfulData = true;
                    ...
                    switch (buffer.getEventCode()) {
                        case LinuxInput.BTN_TOUCH:
                            if (buffer.getEventValue() == 0) {
                                System.out.println("EV_KEY: set all points released ");
                ...
                case LinuxInput.EV_SYN:
                    System.out.println("Syn " + LinuxInput.codeToString(DEBUG_TYPE, (short) buffer.getEventCode()));
                    switch (buffer.getEventCode()) {
